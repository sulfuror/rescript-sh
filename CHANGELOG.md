...
* **Config Template Disruption:** Fixed a regression introduced during visual standardization where `config_template.sh` was accidentally truncated, destroying its `EOF` tag and causing `simple_exclusions` to disappear from the compiled binary. Templates were fully reverted to their original unformatted plaintext to protect configuration parsing.
* **Init Command Infinite Loop:** Fixed a critical bug in `04_core.sh` where `rescript <repo> init` failed to execute on nonexistent local repositories due to the orchestrator ping check recursively blocking creation logic.
...
* **Update Trap Scoping:** Fixed a "unbound variable" crash in the `update` command by removing redundant local `trap` declarations that leaked scope when executed under strict mode (`set -u`).
...
* **Bash Redirection Silent Bug:** Fixed an unexpected Bash 5+ quirk where using `$(<file 2>/dev/null)` silently evaluated to an empty string. This broke the core PID locking system (`02_utils.sh`) and background mount validations, allowing dangerous concurrency. Replaced with robust `$(cat file 2>/dev/null)`.
...
* **I/O Bottlenecks (Process Substitution):** Removed the inefficient process substitution (`<(find ...)`) in the `backup` command used for generating lock file exclusions. Rescript now delegates this directly to Restic's native `--exclude` engine, reducing the blocking I/O bottleneck on massive directory trees.
...
* **JSON Parsing Resilience:** The `size` and `history` commands were entirely refactored to consume Restic's structured JSON output (`--json`) via `awk` instead of parsing standard visual terminal output with fragile regex. This shields the script against crashes caused by Restic UI changes, paths with newlines, or invisible control characters.
...
* **Array Type Enforcements:** Corrected global initialization in `config_template.sh` and `global_config_template.sh` to enforce Bash arrays (`BACKUP_DIR=("$HOME")`), resolving path evaluation bugs for targets containing spaces.
...
* **Null Operator Safety:** Replaced fragile string negation (`[[ ! "$var" ]]`) in the main orchestrator with strict zero-length evaluators (`[[ -z "$var" ]]`), preventing bash syntax evaluation bugs under edge cases.
* **Global Case-Sensitivity Standards:** Unified the strict case-sensitivity state (`shopt -u nocasematch`) globally in `01_globals.sh`, removing 14 repetitive local declarations across the source code. Rescript now enforces standard Linux CLI behavior (e.g., `rescript backup` will intentionally fail if typed as `rescript Backup`).
* **Subshell & Evaluation Optimization:** Removed external subshells (like the inefficient `/usr/bin/whoami` binary usage to check root privileges, now migrated to the native `$EUID` variable) and unnecessary `$(command -v)` evaluations. This prevents process forking and interpreter overhead.
