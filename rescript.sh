...
User options [all user options require arguments]:
	-f			forget: Remove snapshots from the repository
	-g			find: Find a file or directory
	-k			keys: Manage keys [passwords]
	-l			ls: List files in a snapshot	
'
# ------------------------------------------------------------------------------- #
# Options
while getopts ":bcdf:g:hk:l:mnrsuv" o
  do
      case "$o" in
	b|backup)
		if [ -e "$LOCK" ]; then
	...
	r|restore)
		...
		touch "$LOCK"
		trap 'rm -rf "$LOCK"' INT QUIT TERM EXIT
		restic_restore="$HOME/$(basename "$0")-restore-$(date +%s)"
		...
		restic restore latest --target "$restic_restore"
		echo -e "----------------------------------------------------------------------"
		echo -e "$YELLOW""End:""$ENDCOLOR" "$(date)" "         " "$YELLOW""Duration:""$ENDCOLOR" "$((SECONDS / 3600))hrs $(((SECONDS / 60) % 60))min $((SECONDS % 60))sec"
		echo -e "======================================================================"
